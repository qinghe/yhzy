...
class controller_admMessage extends base_adm_controller {
	
	var $view = 'message';
	
	function controller_admMessage() {
		parent::base_adm_controller ();
		$this->dao = get_model('message' );
	}
	/**
	 * 前台留言
	 */
	function actionIndex() {
		$this->set_pager($this->dao, array("address"=>1), 'id desc');
		$this->setLayout ($this->view);
	}
	function actionlianxi() {
		$this->set_pager($this->dao, array("address"=>2), 'id desc');
		$this->setLayout ($this->view);
	}
	function actionToRe() {
